...
const Album = ({images, setImages}) => {
  ...
  useEffect(() => {
    const img = new Image();
    img.onload = function() {
      const magicPadding = this.height / this.width * 100;
      setPadding(magicPadding.toFixed(4) + '%');
    }
      ...
      <div className="flex-container">
        <div className="container">
          <div className="preview-wrapper">  
            <div>
              <div className="adjust-aspect" style={{paddingBottom: `${images.length > 1 ? '56.25%' : padding}`}}>
                        ...
                        {firstImages.map((img, i) => (
                          <div key={i} className="picture">
                            <div className="picture-content">
                              <div className="float">
                                <Button title="x" className="close" onClick={() => removeImage(i)} label={`Remove Image ${i}`} />
                              </div>
                              <img key={i} className="img" src={`${img.url}`} />                    
                            </div>
                          </div>
                        ))}
                      </div>
